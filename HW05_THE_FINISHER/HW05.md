...
``` r
head(Collapsed.Americas.CSV)# I did not use kable to show the table because I want you to see that variable types 
```
...
``` r
ggsave(filename="Redhot.pdf", plot = Redhot, width = 8, height = 8, units = "cm", device = "pdf")

#Explicit provision of the plot object will allow us to extract a single plot for download!(look below)
```
...
This is weird because when I knitr the above I get the photo with the option to download it, but in Github it only has this as a link to the pdf....

![Image](https://github.com/ChadFibke/STAT545-hw-Fibke-Chad/blob/master/HW05_THE_FINISHER/Redhot.pdf)
